...
public class myPoint {
     ...
     * @param _z : z coord
     */
    public myPoint(double _x, double _y, double _z){this.x = _x; this.y = _y; this.z = _z;}
     ...
     */
    public myPoint(myPoint A, double s, myPoint B) {  this(A.x+s*(B.x-A.x),A.y+s*(B.y-A.y),A.z+s*(B.z-A.z)); };
    /**
     * empty constructor
     */
     ...
     */
    public void set(double _x, double _y, double _z){ this.x = _x;  this.y = _y;  this.z = _z; } 
    /**
     * Set this point's values as a copy of the passed point
     * @param p : the point to copy
     * @return this point
     */
    public myPoint set(myPoint p){ this.x = p.x; this.y = p.y; this.z = p.z; return this;}
    /**
     ...
     */
    public myPoint _mult(double n){ this.x *= n; this.y *= n; this.z *= n; return this; }  
    /**
     ...
     */
    public static myPoint _mult(myPoint p, double n){ return new myPoint(p.x * n, p.y * n, p.z * n);}
    /**
     ...
     */
    public static myPoint _mult(myPoint p, myPoint q){ return new myPoint(p.x *q.x, p.y * q.y, p.z * q.z);}
    /**
     * Static method : element-wise multiplication of two points, returning result
     * @param p,q : two points to element-wise multiply
     * @param r : destination for result of element-wise multiplication
     */
    public static void _mult(myPoint p, myPoint q, myPoint r){ myPoint result = new myPoint(p.x *q.x, p.y * q.y, p.z * q.z); r.set(result);}    
    /**
     ...
     */
    public static myPoint _div(myPoint p, double n){ if(n==0) return p; return new myPoint(p.x / n, p.y / n, p.z / n);}
    /**
     ...
     */
    public void _add(myPoint v){ this.x += v.x; this.y += v.y; this.z += v.z;   }
    /**
     ...
     * @return : O + aI
     */
    public static myPoint _add(myPoint O, double a, myVector I){        return new myPoint(O.x+a*I.x,O.y+a*I.y,O.z+a*I.z);}
    /**
     ...
     * @return : O + aI + bJ
     */
    public static myPoint _add(myPoint O, double a, myVector I, double b, myVector J) {                            return new myPoint(O.x+a*I.x+b*J.x,O.y+a*I.y+b*J.y,O.z+a*I.z+b*J.z);}
    /**
     ...
     * @return : O + aI + bJ + cK
     */
    public static myPoint _add(myPoint O, double a, myVector I, double b, myVector J, double c, myVector K) {    return new myPoint(O.x+a*I.x+b*J.x+c*K.x,O.y+a*I.y+b*J.y+c*K.y,O.z+a*I.z+b*J.z+c*K.z);} 
    /**
     ...
     */
    public static void _add(myPoint p, myPoint q, myPoint r){ myPoint result = new myPoint(p.x + q.x, p.y + q.y, p.z + q.z); r.set(result);}  
    /**
     ...
     */
    public void _sub(double _x, double _y, double _z){ this.x -= _x; this.y -= _y; this.z -= _z;  }
    /**
     * element-wise subtract passed point from this point, making this point equal to result. 
     * @param v point to subtract this
     */    
    public void _sub(myPoint v){ this.x -= v.x; this.y -= v.y; this.z -= v.z;  }
    /**
     ...
     */
    public static void _sub(myPoint p, myPoint q, myPoint r){ myPoint result = new myPoint(p.x - q.x, p.y - q.y, p.z - q.z); r.set(result);}     
     ...
     */    
    public double L1Dist(myPoint q){return Math.abs((this.x - q.x)) + Math.abs((this.y - q.y)) + Math.abs((this.z - q.z)); }
    /**
     ...
     */    
    public static double _L1Dist(myPoint q, myPoint r){ return q.L1Dist(r);}
     ...
     */
    public double sqrDist(myPoint q){ double dx=(this.x-q.x), dy=(this.y-q.y), dz=(this.z-q.z);return ((dx*dx) + (dy*dy) + (dz*dz)); }
    /**
     ...
     */
    public static double _SqrDist(myPoint pt, myPoint pts){ double dx=(pts.x-pt.x), dy=(pts.y-pt.y), dz=(pts.z-pt.z);return ((dx*dx) + (dy*dy) + (dz*dz)); } 
    /**
     ...
     */
    public double dist(myPoint q){ double dx=(this.x-q.x), dy=(this.y-q.y), dz=(this.z-q.z);return Math.sqrt(((dx*dx) + (dy*dy) + (dz*dz)));}
    /**
     ...
     */
    public double dist(double qx, double qy, double qz){ double dx=(this.x-qx), dy=(this.y-qy), dz=(this.z-qz);return Math.sqrt(((dx*dx) + (dy*dy) + (dz*dz)));}
    /**
     ...
     * @return
     */
    public static final int _findClosestPtToPt(myPoint[] pts, myPoint pt, double[] d) {
        int resIdx = -1; 
        double minSqDist = 99999999, _sqrD;
        for(int i=0; i<pts.length; ++i){_sqrD = myPoint._SqrDist(pt,pts[i]);if(_sqrD < minSqDist){minSqDist = _sqrD; resIdx = i;}}
        d[0] =  Math.sqrt(minSqDist);            
        return resIdx;
    }//_findClosestPtToPt 
     ...
     */
    public myPoint rotMeAroundPt(myPoint C, myPoint P, myPoint R) {
     ...
     * @return
     */    
    public boolean clickIn(myPoint p, double eps) { return(dist(p) < eps);}
